...
> **✅ DONE on `rebuild/v2` (one squashed commit `c435219`) — device-accepted.** Built across contracts/DB (strict EDL, montage tables, migration 0005) · `infra/remotion` (composition + Node render driver + 4 synth tracks) · worker (scoring + beat-aligned EDL builder + `render-montage` BullMQ queue + `RemotionRenderer` Node-child spawn + video-poster) · API (5 routes) · mobile (generating + review screens, `expo-video`). Verified: tsc clean all pkgs; `bun test` **176/0** incl. a real ~72s Remotion render gate (ffprobe 1080×1920/30fps/30.06s h264); live e2e proved the queue+S3 seam + publish/idempotent-republish; on-device generate→review(plays)→publish accepted. Adversarial review (9 findings) fixed pre-commit. **Viewing the published montage on another group member's account = the feed, owned by M8** (M7 writes the `montage_group_visibility` rows it reads).
