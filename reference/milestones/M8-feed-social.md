...
- **Comment length / rate limits:** spec §11 mandates rate-limiting comments but not exact numbers. *Default:* `text` ≤ 500 chars; comment POST ≤ ~10/min/user, reaction set/clear ≤ ~30/min/user (env-configurable, mirroring the §5 OTP-cap-configurable lesson for deterministic CI). Confirm numbers with product.
- **Comment preview depth:** *Default* 2 latest active+block-clean comments on the card. Confirm with design (Ember Feed+Comments screen).
